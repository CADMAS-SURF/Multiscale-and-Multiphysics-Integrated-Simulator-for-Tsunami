      SUBROUTINE MPCTRNSG(IMPC,KN,ND,NDF,NC,RAMBDA,NRAMAX,INDOF,INDMPC
     &                   ,MPCF,RMPC,ELHM12,ELHM21,IDCR,NCR)
C
      IMPLICIT REAL*8(A-H,O-Z)
      DIMENSION KN(ND),IDCR(2,*),RAMBDA(NC,*),INDOF(6,*),INDMPC(2,6,*)
     &         ,MPCF(2,*),RMPC(*),WK(NC*ND),ELHM12(*),ELHM21(*)
C-----------------------------------------------------------------------
      NCR=0
      DO 100 I=1,ND
      DO 100 J=1,NDF
...
C
      WK(:) = ELHM12(1:NC*ND)
      CALL ATXB(ELHM12,RAMBDA,WK,NC,NCR,ND)
      WK(:) = ELHM21(1:ND*NC)
      CALL AXB(ELHM21,WK,RAMBDA,ND,NC,NCR)
C
  900 RETURN
      END
