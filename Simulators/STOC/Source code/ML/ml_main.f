...
C======================================================================
     ...
     $                   LLWALL,LLWALP,LLWALB,LLOFL,HHOFL
      use mod_psi,only: init_psi,fin_psi
      IMPLICIT NONE
C
      INCLUDE 'DOMAIN.h'
      INCLUDE 'FILE.h'
...
C
     ...
     $   HU(:,:,:),HV(:,:,:),HW(:,:,:),
     $   TT(:,:,:),CC(:,:,:),Q2(:,:,:),QL(:,:,:),
     $   DKXX(:,:,:),DKXY(:,:,:),DKYY(:,:,:),DKM(:,:,:),DKH(:,:,:),
     $   TMU(:,:,:),FF(:,:,:),RHOW(:,:,:),VLEND(:,:,:),
     $   TMEND(:,:,:),FREND(:,:,:),FRIC(:,:,:),RWWB(:,:,:),RWWF(:,:,:)
     ...
     $   GX_ML(:,:,:),GY_ML(:,:,:),GZ_ML(:,:,:),
     $   UU_ML(:,:,:),VV_ML(:,:,:),WW_ML(:,:,:),
     $   TT_ML(:,:,:),CC_ML(:,:,:),Q2_ML(:,:,:),QL_ML(:,:,:),
     $   UUBCN(:,:,:),VVBCN(:,:,:),WWBCN(:,:,:),
     $   TTBCN(:,:,:),CCBCN(:,:,:),Q2BCN(:,:,:),QLBCN(:,:,:)
C
      REAL(8),ALLOCATABLE::HTDST1(:,:),HTDST2(:,:)
      INTEGER,ALLOCATABLE::IDST(:,:)
      REAL(8),ALLOCATABLE::TMDST(:,:)
...
C
      ...
      CALL INCNCT(MLNS)
      CALL FTIMER(2,1)
      write(lp,*) 'nrank,nmfile=',nrank,nmfile(nrank+1)
C
...
C
C      OPEN(INP,FILE='data.in',STATUS='OLD',FORM='FORMATTED',ERR=900)
...
C
      WRITE(LP,*) '+----------------------------------+'
      WRITE(LP,*) '|  PROGRAM STOC(ML) VER.07.01.02v  |'
      WRITE(LP,*) '+----------------------------------+'
...
C
      IF(RFILE(3).GT.0.0D0.AND.NUMPE(1,NRANK+1).GT.1) THEN
        CALL ERRMSG('ML_MAIN',6001)
        WRITE(LP,*) 'OUTPUT BOUNDARY FILE FLAG ON'
        WRITE(LP,*) 'NUMBER OF CHILID-PE =',NUMPE(1,NRANK+1)
        CALL ABORT1('')
      END IF
C
C ... 境界条件出力用ファイルを開く
      IF(RFILE(3).GT.0.0D0) THEN
        IF(ICHILD.GE.0) THEN
           call flnam('.bco')
          WRITE(LP,*) CFLNM
          OPEN(IFLBO,FILE=trim(CFLNM),STATUS='NEW',
     $         FORM='UNFORMATTED',ERR=902)
        ELSE
          CALL ERRMSG('ML_MAIN',6002)
          WRITE(LP,*) 'CHILD PE NOT FOUND ( STOP )'
          CALL ABORT1('')
        END IF
      END IF
C
C ... 境界条件入力用ファイルを開く
      IF(IPFLG.GT.0.0D0) THEN
         call flnam('.bci')
         WRITE(LP,*) CFLNM
         OPEN(IFLBI,FILE=trim(CFLNM),STATUS='OLD',
     $        FORM='UNFORMATTED',ERR=903)
      END IF
      IF(IPFLG.GT.0) NESTFL=1
C
...
C     親の格子数を受信する。
     ...
     $                  comm_model,IREQ3,IERROR)
         ...
         CALL MPI_WAIT(IREQ3,ISTAT,IERROR)
         ELSE
         READ(IFLBI) MX_ML,MY_ML,MZ_ML
         write(6,*) 'MX_ML,MY_ML,MZ_ML=',MX_ML,MY_ML,MZ_ML
         END IF  
         CALL FTIMER(6,1)
C
...
C
     ...
     *                  MPI_ANY_TAG,comm_model,IREQ3,IERROR)
         ...
         CALL MPI_WAIT(IREQ3,ISTAT,IERROR)
         ELSE
         READ(IFLBI) XC_ML,YC_ML,ZC_ML
cccccc
      write(6,*) 'XC_ML,YC_ML,ZC_ML'
      write(6,*) 'xc_ml(1-mx_ml-1)=',xc_ml(1,1),xc_ml(1,mx_ml-1)
      write(6,*) 'yc_ml(1-my_ml-1)=',yc_ml(1,1),yc_ml(1,my_ml-1)
      write(6,*) 'zc_ml(1-mz_ml-1)=',zc_ml(1,1),zc_ml(1,mz_ml-1)
         END IF
         CALL FTIMER(8,1)
      ENDIF
C
...
C
         IF(RFILE(3).GT.0.0D0) THEN
           WRITE(IFLBO) IWES,IEAS,JSOU,JNOR,KBOT,KTOP
         END IF
C
         CALL FTIMER(12,1)
C
...
C
C        自動ネスティング設定
         CALL AUTONEST(IWES,IEAS,JSOU,JNOR,KBOT,KTOP)
C
      ENDIF
C
C     親のオーバーラップ幅と計算範囲を受信する。
     ...
     $                 IREQ7,IERROR)
         ...
         CALL FTIMER(12,1)
         ELSE
         READ(IFLBI) IWES_ML,IEAS_ML,JSOU_ML,JNOR_ML,KBOT_ML,KTOP_ML
ccccc
      write(6,*) 'IWES_ML,IEAS_ML=',IWES_ML,IEAS_ML
      write(6,*) 'JSOU_ML,JNOR_ML=',JSOU_ML,JNOR_ML
      write(6,*) 'KBOT_ML,KTOP_ML~',KBOT_ML,KTOP_ML
         END IF
      END IF
C
C
...
C
     ...
     $         INDW(MX,MY,MZ),WRK1(MX,MY,MZ1),WRK2(MX,MY,MZ1),
     $         WRK3(MX,MY,MZ1),STAT=IERR)
      IF(IERR.NE.0) THEN
         CALL ERRMSG('ML_MAIN',6008)
...
C
     ...
     $         Q2(MX,MY,MZ),QL(MX,MY,MZ),DKXX(MX,MY,MZ),
     $         DKXY(MX,MY,MZ),DKYY(MX,MY,MZ),DKM(MX,MY,MZ),
     $         DKH(MX,MY,MZ),TMU(MX,MY,MZ),STAT=IERR)
      IF(IERR.NE.0) THEN
         CALL ERRMSG('ML_MAIN',6016)
...
C
     ...
     $         RWWB(MX,MY,9),RWWF(MX,MY,9),HH(MX,MY),
...
C
      IF( LSTOCDS.EQ.1 ) THEN
         ALLOCATE(HTDST1(MX,MY),HTDST2(MX,MY),IDST(MX,MY),
     $            TMDST(MX,MY),KBLC(MX,MY),STAT=IERR)
      ELSE
         ALLOCATE(HTDST1(1,1),HTDST2(1,1),IDST(1,1),
     $            TMDST(1,1),KBLC(1,1),STAT=IERR)
      ENDIF
      IF(IERR.NE.0) THEN
         CALL ERRMSG('ML_MAIN',6019)
         WRITE(LP,*) 'CANNOT ALLOCATE HTDST1,...'
         CALL ABORT1('')
      END IF
      HTDST1=0.d0
      HTDST2=0.d0
...
C
     ...
     $              ZBD_ML(1,1),STAT=IERR)
         END IF
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6024)
            WRITE(LP,*) 'CANNOT ALLOCATE CSD_ML,...'
            CALL ABORT1('')
         ENDIF
C
         IF(LTURB.EQ.3) THEN
           ALLOCATE(Q2_ML(MX_PARNT,MY_PARNT,MZ_PARNT),
     $              QL_ML(MX_PARNT,MY_PARNT,MZ_PARNT),STAT=IERR)
         ELSE
           ALLOCATE(Q2_ML(1,1,1),
     $              QL_ML(1,1,1),STAT=IERR)
         END IF
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6025)
            WRITE(LP,*) 'CANNOT ALLOCATE Q2_ML,...'
            CALL ABORT1('')
         ENDIF
C
         ALLOCATE(HH_ML(MX_PARNT,MY_PARNT),
     $            KG_ML(MX_PARNT,MY_PARNT),
     $            KF_ML(MX_PARNT,MY_PARNT),
     $            HDEP_ML(MX_PARNT,MY_PARNT),STAT=IERR)
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6026)
            WRITE(LP,*) 'CANNOT ALLOCATE HH_ML,...'
            CALL ABORT1('')
         ENDIF
C
         NXY = MAX(MX,MY)
...
C
     ...
     $              ZBDBCN(1,1),STAT=IERR)
         END IF
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6028)
            WRITE(LP,*) 'CANNOT ALLOCATE CSDBCN,...'
            CALL ABORT1('')
         ENDIF
C
         IF(LTURB.EQ.3) THEN
           ALLOCATE(Q2BCN(NXY,MZ,4),
     $              QLBCN(NXY,MZ,4),STAT=IERR)
         ELSE
           ALLOCATE(Q2BCN(1,1,1),
     $              QLBCN(1,1,1),STAT=IERR)
         END IF
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6029)
            WRITE(LP,*) 'CANNOT ALLOCATE Q2BCN,...'
            CALL ABORT1('')
         ENDIF
C
         ALLOCATE(HHBCN(MX,MY),
     $            HHW(MX,MY),STAT=IERR)
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6030)
            WRITE(LP,*) 'CANNOT ALLOCATE HHBCN,...'
            CALL ABORT1('')
         ENDIF
      ELSE
         ALLOCATE(UU_ML(1,1,1),VV_ML(1,1,1),WW_ML(1,1,1),
     $            TT_ML(1,1,1),CC_ML(1,1,1),CSD_ML(1,1,1),
     $            ZBD_ML(1,1),Q2_ML(1,1,1),QL_ML(1,1,1),
     ...
     $            Q2BCN(1,1,1),QLBCN(1,1,1),
     $            HHBCN(1,1),HHW(1,1),STAT=IERR)
         IF(IERR.NE.0) THEN
            CALL ERRMSG('ML_MAIN',6031)
            WRITE(LP,*) 'CANNOT ALLOCATE UU_ML,...'
            CALL ABORT1('')
         ENDIF
      END IF
...
C
      CALL FTIMER(17,0)
      CALL MKINIT(UU,VV,WW,PP,TT,CC,Q2,QL,TMU,
...
C20111124            IF(ICHILD.GE.0) THEN
     ...
     3                       ICHILD)
        ...
        END IF
        IF(IERR1.NE.0) THEN
          CALL ERRMSG('ML_MAIN',6032)
          CFLNM(IFLNM-3:IFLNM) = '.str'
          WRITE(6,*)  'FILE DATA UNMACTHED = ',CFLNM(1:IFLNM)
...
C
     ...
     1                Q2,QL,TMU,WX,WY,CD,PATM,HH,KF,KP,IFLAG)
C
      IF((IPARNT.GE.0.OR.IPFLG.NE.0).AND.LSTART.EQ.0) THEN
C
         CALL FTIMER(23,0)
...
C
      IF(LTURB.EQ.3) THEN
        ALLOCATE(Q2N(MX,MY,MZ),QLN(MX,MY,MZ),STAT=IERR)
      ELSE
        ALLOCATE(Q2N(1,1,1),QLN(1,1,1),STAT=IERR)
      END IF
      IF(IERR.NE.0) THEN
         CALL ERRMSG('ML_MAIN',6034)
         WRITE(LP,*) 'CANNOT ALLOCATE Q2N,...'
         CALL ABORT1('')
      END IF
      Q2N=0.d0
      QLN=0.d0
C
      ALLOCATE(DP(MX,MY,MZ),STAT=IERR)
      IF(IERR.NE.0) THEN
         CALL ERRMSG('ML_MAIN',6035)
         WRITE(LP,*) 'CANNOT ALLOCATE DP'
         CALL ABORT1('')
      END IF
      DP=0.d0
C
...
C
     ...
     $              TMUBW,TIMBW,
     $              PATM,DPS,QQ,QW,WX,WY,UN,VN,WN,TN,CN,Q2N,QLN,
     ...
     $              TMEND,FREND,FRIC,HDEP,AMNG,CD,HX,HY,HHW,RWWB,RWWF,
     $              INDP,INDU,INDV,INDW,
...
C ... ファイルオープンエラー
  900 CONTINUE
      CALL ERRMSG('ML_MAIN',6040)
      WRITE(LP,*) 'FILE OPEN ERROR: DEBUG OUTPUT FILE'
      WRITE(LP,*) 'FILE NUMBER=',IFLDB
      CALL ABORT1('')
C
  901 CONTINUE
      CALL ERRMSG('ML_MAIN',6041)
      WRITE(LP,*) 'FILE OPEN ERROR: TC-INITIAL FILE'
      WRITE(LP,*) 'FILE NUMBER=',IFINI
      CALL ABORT1('')
C
  902 CONTINUE
      CALL ERRMSG('ML_MAIN',6042)
      WRITE(LP,*) 'FILE OPEN ERROR: BOUNDARY OUTPUT FILE'
      WRITE(LP,*) 'FILE NUMBER=',IFLBO
      CALL ABORT1('')
C
  903 CONTINUE
      CALL ERRMSG('ML_MAIN',6043)
      WRITE(LP,*) 'FILE OPEN ERROR: BOUNDARY INPUT FILE'
      WRITE(LP,*) 'FILE NUMBER=',IFLBI
      CALL ABORT1('')
C
      END
