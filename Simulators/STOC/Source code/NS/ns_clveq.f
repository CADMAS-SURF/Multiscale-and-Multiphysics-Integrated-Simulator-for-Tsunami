     ...
     $                 COE1D,COE2D,VP,TMU,TMUBW,HH,FF,FALLWY,DHY,
     $                 INDP,INDU,INDV,INDW,
...
C     (1) 境界面上のVを設定する
C----------------------------------------------------------------------
      CALL BCVVXZ(VVX,VVZ,UU,VV,WW,TMUX,XCP,YC,ZC,LLWALL,INDP,
...
C
      CALL FLUXVX(FU,UU,VV,HU,VVX,FF,TMUX,XC,YC,XCP,GV,GX,GV0,GX0,
     $            INDU,INDV,KF)
C
      CALL FLUXVY(FV,VV,HV,TMUX,FF,YC,GV,GY,GV0,GY0,INDV,LLWALB,KF)
C
...
C----------------------------------------------------------------------
      CALL FLUXVP(DV,UU,VV,WW,TMUX,XC,YC,ZC,INDV,LLWALP)
C
C
C----------------------------------------------------------------------
C     (4) 圧力差を計算する
...
C
C----------------------------------------------------------------------
C     (5) 仮流速のy方向成分を計算する(自由流入出境界上の点は除く)
C----------------------------------------------------------------------
