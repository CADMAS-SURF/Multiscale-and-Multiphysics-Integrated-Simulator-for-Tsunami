...
CD    BCU(NUMB,3)      : OUT : R*8 : x方向流速の境界値
CD    BCV(NUMB,3)      : OUT : R*8 : y方向流速の境界値
CD    BCW(NUMB,3)      : OUT : R*8 : z方向流速の境界値
...
CD    BCK(NUMB,3)      : OUT : R*8 : 乱流エネルギの境界値
CD    BCE(NUMB,3)      : OUT : R*8 : 乱流エネルギ散逸の境界値
...
C       OPEN(MFILRE,ERR=9010,FILE='data.res',
        OPEN(MFILRE,ERR=9010,FILE=TRIM(MGNAME(MGRANK+1))//'.res',
C----------------------------------------------------for MG/2FC coupling
     &       STATUS='OLD',FORM='UNFORMATTED' )
      ELSE
        WRITE(TEXTP,'(I5.5)') MYRANK
C----------------------------------------------------for MG/2FC coupling
C       OPEN(MFILRE,ERR=9010,FILE='data.res'//TEXTP,
        OPEN(MFILRE,ERR=9010,FILE=TRIM(MGNAME(MGRANK+1))//'.res'//TEXTP,
C----------------------------------------------------for MG/2FC coupling
     &       STATUS='OLD',FORM='UNFORMATTED' )
      ENDIF
      IREFIL=MFILRE
...
CMVO  IF (NUMB.NE.I4) /* 移動構造物のための変更 */
      IF (NUMB.GT.I4)
...
CD      -- 流速を読み込む --
     ...
     &          (((UU(I,J,K),I=1,NUMI),J=1,NUMJ),K=1,NUMK)
        READ(IREFIL,ERR=9020) ((BCU(L1,L2),L1=1,NUMB),L2=1,3)
        READ(IREFIL,ERR=9020)
     &          (((VV(I,J,K),I=1,NUMI),J=1,NUMJ),K=1,NUMK)
        READ(IREFIL,ERR=9020) ((BCV(L1,L2),L1=1,NUMB),L2=1,3)
        READ(IREFIL,ERR=9020)
     &          (((WW(I,J,K),I=1,NUMI),J=1,NUMJ),K=1,NUMK)
        READ(IREFIL,ERR=9020) ((BCW(L1,L2),L1=1,NUMB),L2=1,3)

CD      -- 圧力を読み込む --
        READ(IREFIL,ERR=9020)
     &          (((PP(I,J,K),I=1,NUMI),J=1,NUMJ),K=1,NUMK)
        READ(IREFIL,ERR=9020) ((BCP(L1,L2),L1=1,NUMB),L2=1,3)
...
CD      -- 乱流エネルギを読み込む --
     ...
     &               (((AK(I,J,K),I=1,NUMI),J=1,NUMJ),K=1,NUMK)
          READ(IREFIL,ERR=9020) ((BCK(L1,L2),L1=1,NUMB),L2=1,3)
        ENDIF
...
CD      -- 乱流エネルギ散逸を読み込む --
     ...
     &               (((AE(I,J,K),I=1,NUMI),J=1,NUMJ),K=1,NUMK)
          READ(IREFIL,ERR=9020) ((BCE(L1,L2),L1=1,NUMB),L2=1,3)
        ENDIF
...
CD    -- 動粘性係数等の設定 --
      ...
      IF (LEQT.NE.0) CALL VF_CLM00(ANUT ,ALM ,NF ,FF)
...
C==== ファイル関連エラー処理 =========================================
 ...
 9010 CONTINUE
C----------------------------------------------------for MG/2FC coupling
C     CALL VF_A2ERR('VF_IR1INI','CAN NOT OPEN (data.res).')
      CALL VF_A2ERR('VF_IR1INI','CAN NOT OPEN ('
...
C----------------------------------------------------for MG/2FC coupling
 ...
 9020 CONTINUE
C----------------------------------------------------for MG/2FC coupling
C     CALL VF_A2ERR('VF_IR1INI','READ ERROR (data.res).')
      CALL VF_A2ERR('VF_IR1INI','READ ERROR ('
     &                //TRIM(MGNAME(MGRANK+1))//'.res).')
C----------------------------------------------------for MG/2FC coupling
      GOTO 9999
