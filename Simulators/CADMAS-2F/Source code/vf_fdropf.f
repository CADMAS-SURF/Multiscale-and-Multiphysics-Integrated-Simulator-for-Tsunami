...
C               * 近傍に流体セルがあればそれを集める
     ...
     &              (NF(I1,J,K).GT.-1 .OR. NF(I,J,K1).GT.-1)) THEN
                  IP=IP+1
     ...
     &              (NF(I1,J,K).GT.-1 .OR. NF(I,J1,K).GT.-1)) THEN
                  IP=IP+1
     ...
     &              (NF(I,J1,K).GT.-1 .OR. NF(I,J,K1).GT.-1)) THEN
                  IP=IP+1
...
C               * 流体セルがなければ、表面セルを集める
                IF (IP.EQ.0) THEN
                  I1=I-1
                  IF (NF(I1,J,K).GT.-1 .AND. NF(I1,J,K).NE.8) THEN
                    IP=IP+1
                  ...
                  ENDIF
                  K1=K-1
                  IF (NF(I,J,K1).GT.-1 .AND. NF(I,J,K1).NE.8) THEN
                    IP=IP+1
                  ...
                  ENDIF
                  J1=J-1
                  IF (NF(I,J1,K).GT.-1 .AND. NF(I,J1,K).NE.8) THEN
                    IP=IP+1
...
C             * 水と重み付き値を移動
     ...
     &            NF(I1,J,K).GT.-1 .AND. SG*DROPUU(I,J,K).GT.0.0D0) THEN
...
C             * 水と重み付き値を移動
              DT=0.0D0
              IF (FF(I,J,K).GT.FLOWER .AND. NF(I,J,K).EQ.8 .AND.
     &            NF(I,J1,K).GT.-1 .AND. SG*DROPVV(I,J,K).GT.0.0D0) THEN
...
C               -- 水路天井にあたるようなら止める --
                IF (L.EQ.1) THEN
                  X=SQRT(2.0D0*GRZ0*ZZ(2,K))
                  IF (NF(I,J,K+1).LE.-1 .AND. DROPWW(I,J,K).GT.X) THEN
                    DROPUU(I,J,K)=0.0D0
                    DROPVV(I,J,K)=0.0D0
                    DROPWW(I,J,K)=0.0D0
                  ENDIF
