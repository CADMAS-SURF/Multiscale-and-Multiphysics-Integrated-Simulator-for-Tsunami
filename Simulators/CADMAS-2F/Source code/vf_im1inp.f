...
CDT   VF_IM1INP:マトリクスデータファイルを読み込む
...
CD    -- ファイルをオープンし、データ数等を読み込む --
      ...
      IMTFIL=0
C----------------------------------------------------for MG/2FC coupling
C     OPEN(MFILMT,ERR=9010,FILE='data.mtb',
      OPEN(MFILMT,ERR=9010,FILE=TRIM(MGNAME(MGRANK+1))//'.mtb',
C----------------------------------------------------for MG/2FC coupling
...
C     * マトリクスデータのタイプ
      IP=IP+1
      IF (IP.GT.NWD) THEN
        CALL VF_ZGETLN(IS,IE,MAXWDS,NWD,IMTFIL,IEOF,TEXT)
...
C==== ファイル関連エラー処理 =========================================
 ...
 9010 CONTINUE
C----------------------------------------------------for MG/2FC coupling
C     CALL VF_A2ERR('VF_IM1INP','CAN NOT OPEN (data.mtb).')
      CALL VF_A2ERR('VF_IM1INP','CAN NOT OPEN ('
     &                  //TRIM(MGNAME(MGRANK+1))//'.mtb).')
C----------------------------------------------------for MG/2FC coupling
      GOTO 9999
