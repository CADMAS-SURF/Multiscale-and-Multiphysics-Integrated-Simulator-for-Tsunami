...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      DIMENSION FX  (NUMI,NUMJ,NUMK),FY  (NUMI,NUMJ,NUMK)
      DIMENSION FZ  (NUMI,NUMJ,NUMK)
      DIMENSION UUO (NUMI,NUMJ,NUMK),VVO (NUMI,NUMJ,NUMK)
      DIMENSION WWO (NUMI,NUMJ,NUMK)
...
CD    -- x方向仮流速の計算 --
      DO 120 K=2,NUMK-1
        DO 110 J=MYJS,MYJE
          DO 100 I=IA,IB
C2F         IF (INDX(I,J,K).EQ.0 .AND. NF(I-1,J,K)*NF(I,J,K).EQ.0) THEN
            IF (INDX(I,J,K).EQ.0) THEN
              GV =XX(6,I)*(XX(2,I)*GLV (I,J,K)+XX(2,I-1)*GLV (I-1,J,K))
              GV0=XX(6,I)*(XX(2,I)*GLV0(I,J,K)+XX(2,I-1)*GLV0(I-1,J,K))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              DRDTU=XX(6,I)*(XX(2,I  )*DRHODT(I  ,J,K)
     &                      +XX(2,I-1)*DRHODT(I-1,J,K))
     &                     *(1.0D0 - FX(I,J,K))*UU(I,J,K)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!            UU(I,J,K)=(GV*UU(I,J,K)+DTNOW*DU)/GV0
              UU(I,J,K)=(GV*UUO(I,J,K)+DTNOW*(DU+DRDTU))/GV0
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C@@@@@@@@@
...
CD    -- y方向仮流速の計算 --
      DO 220 K=2,NUMK-1
        DO 210 J=JA,JB
          DO 200 I=MYIS,MYIE
C2F         IF (INDY(I,J,K).EQ.0 .AND. NF(I,J-1,K)*NF(I,J,K).EQ.0) THEN
            IF (INDY(I,J,K).EQ.0) THEN
              GV =YY(6,J)*(YY(2,J)*GLV (I,J,K)+YY(2,J-1)*GLV (I,J-1,K))
              GV0=YY(6,J)*(YY(2,J)*GLV0(I,J,K)+YY(2,J-1)*GLV0(I,J-1,K))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              DRDTV=YY(6,J)*(YY(2,J  )*DRHODT(I,J  ,K)
     &                      +YY(2,J-1)*DRHODT(I,J-1,K))
     &                     *(1.0D0 - FY(I,J,K))*VV(I,J,K)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!            VV(I,J,K)=(GV*VV(I,J,K)+DTNOW*DV)/GV0
              VV(I,J,K)=(GV*VVO(I,J,K)+DTNOW*(DV+DRDTV))/GV0
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C@@@@@@@@@
...
CD    -- z方向仮流速の計算 --
      DO 320 K=3,NUMK-1
        DO 310 J=MYJS,MYJE
          DO 300 I=MYIS,MYIE
C2F         IF (INDZ(I,J,K).EQ.0 .AND. NF(I,J,K-1)*NF(I,J,K).EQ.0) THEN
            IF (INDZ(I,J,K).EQ.0) THEN
              GV =ZZ(6,K)*(ZZ(2,K)*GLV (I,J,K)+ZZ(2,K-1)*GLV (I,J,K-1))
              GV0=ZZ(6,K)*(ZZ(2,K)*GLV0(I,J,K)+ZZ(2,K-1)*GLV0(I,J,K-1))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              DRDTW=ZZ(6,K)*(ZZ(2,K  )*DRHODT(I,J,K  )
     &                      +ZZ(2,K-1)*DRHODT(I,J,K-1))
     &                     *(1.0D0 - FZ(I,J,K))*WW(I,J,K)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!            WW(I,J,K)=(GV*WW(I,J,K)+DTNOW*DW)/GV0
              WW(I,J,K)=(GV*WWO(I,J,K)+DTNOW*(DW+DRDTW))/GV0
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C@@@@@@@@@
