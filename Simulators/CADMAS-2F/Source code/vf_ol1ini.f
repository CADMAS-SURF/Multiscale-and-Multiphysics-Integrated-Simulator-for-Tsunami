...
CD    INDBC(NUMB,LEQC) : IN : I*4 : 濃度の境界条件
      ...
      DIMENSION BCF(NUMB),BCK(NUMB,3),BCE(NUMB,3),BCT(NUMB),BCTI(2,NUMB)
...
CD    -- 物性値関連情報を出力 --
      ...
      WRITE(ILPFIL,9540) 'DENSITY [KG/M3] =',RHO0(1),RHO0(2)
      WRITE(ILPFIL,9540) 'K-VISC   [M2/S] =',ANU0(1),ANU0(2)
 ...
 120  CONTINUE
      ...
      IF (LEQK.NE.0) THEN
        WRITE(ILPFIL,9540) '       K[M2/S2] =',AKINIK(1),AKINIK(2)
        WRITE(ILPFIL,9540) '       E[M2/S3] =',AKINIE(1),AKINIE(2)
      ENDIF
      IF (LEQT.NE.0) THEN
...
CD    -- モデル関連情報を出力 --
      ...
      DO 200 JD=1,4
        IF (IBCTYP(1,JD).EQ.1) THEN
          ...
          IF (ABS(BCTYP(9,JD)).GT.ZERO) THEN
...
CD    -- 障害物関連情報を出力 --
      ...
      IF (ILPON(4).NE.0) THEN
        ...
        CALL VF_OL3DR(CD0,0)
      ENDIF
CSTR  IF (ILPON(5).NE.0 .AND. IPRNT.LE.1) THEN
      IF (ILPON(5).NE.0 .AND. IPRNT.LE.1 .AND. 
     &    ICPL.LE.1 .AND. ISTM.EQ.0) THEN
        ...
        CALL VF_OL3DR(GGZ,1)
      ENDIF
CSTR  IF (ILPON(6).NE.0 .AND. IPRNT.LE.1) THEN
      IF (ILPON(6).NE.0 .AND. IPRNT.LE.1 .AND. 
     &    ICPL.LE.1 .AND. ISTM.EQ.0) THEN
...
CD    -- 境界条件関連情報を出力 --
     ...
     &                 CALL VF_A2ERR('VF_OL1INI','UNDEFINED.')
        IF (LEQK.NE.0) THEN
          IF (INDBK(1,L).EQ.0) CALL VF_A2ERR('VF_OL1INI','UNDEFINED.')
        ENDIF
        IF (LEQK.NE.0) THEN
          IF (INDBE(1,L).EQ.0) CALL VF_A2ERR('VF_OL1INI','UNDEFINED.')
        ENDIF
        IF (LEQT.NE.0) THEN
