...
CBUG  DO 220 K=2,NUMK
      DO 220 K=2,NUMK-1
        DO 210 J=MYJS,MYJE+1
          DO 200 I=LA,LB
C2F         IC=NF(I-1,J-1,K)*NF(I,J-1,K)*NF(I-1,J,K)*NF(I,J,K)
            IC=INDC(I-1,J-1,K)*INDC(I,J-1,K)*INDC(I-1,J,K)*INDC(I,J,K)
            IT=INDX(I,J  ,K)
            IB=INDX(I,J-1,K)
...
CBUG            IF (NF(I,J,K).NE.8) THEN
                IF (NF(I,J,K).NE.8 .AND. NF(I,J-1,K).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
                IF (INDB(LGBF,IS).EQ.1 .OR. INDB(LGBF,IS).EQ.4) THEN
                  ...
                  ENDIF
                  BR2=BR
CSTR            ELSEIF (INDB(LGBF,IS).EQ.2 .AND. IS.GT.NUMB0) THEN
                ELSEIF (INDB(LGBF,IS).EQ.2 .AND. IS.GT.NUMB0
     &                  .AND. ICPL.EQ.0 .AND. ISTM.EQ.0) THEN
                  IF (INDB(2,IS).EQ.3) THEN
                    BR=UU(I,J  ,K)
                  ELSE
                    BR=UU(I,J-1,K)
...
C             * 負側の流速値
              IS=INDY(I-1,J,K)
              IF (IS.GE.1) THEN
CBUG            IF (NF(I-1,J,K).NE.8) THEN
                IF (NF(I-1,J,K).NE.8 .AND. NF(I-1,J-1,K).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
...
C             ? 線形補間
              GG=XX(6,I)*(XX(2,I-1)*GGY(I,J,K)+XX(2,I)*GGY(I-1,J,K))
CMVO          FG=XX(6,I)*(XX(2,I)*BR+XX(2,I-1)*BL)
              FG=XX(6,I)*(XX(2,I)*BR2+XX(2,I-1)*BL2)
              IF     (NF(I,J  ,K).LE.-1) THEN
                ANR=ANU(I,J-1,K)
              ELSEIF (NF(I,J-1,K).LE.-1) THEN
                ANR=ANU(I,J  ,K)
              ELSE
                ANR=YY(6,J)*( YY(2,J-1)*ANU(I,J  ,K)
     &                       +YY(2,J  )*ANU(I,J-1,K))
              ENDIF
              IF     (NF(I-1,J  ,K).LE.-1) THEN
                ANL=ANU(I-1,J-1,K)
              ELSEIF (NF(I-1,J-1,K).LE.-1) THEN
                ANL=ANU(I-1,J  ,K)
              ELSE
                ANL=YY(6,J)*( YY(2,J-1)*ANU(I-1,J  ,K)
     &                       +YY(2,J  )*ANU(I-1,J-1,K))
...
CD    -- (Gz*w)*uとGz*Nu*(du/dz+dw/dx)の計算 --
      DO 320 K=2,NUMK
        DO 310 J=MYJS,MYJE
          DO 300 I=LA,LB
C2F         IC=NF(I-1,J,K-1)*NF(I,J,K-1)*NF(I-1,J,K)*NF(I,J,K)
            IC=INDC(I-1,J,K-1)*INDC(I,J,K-1)*INDC(I-1,J,K)*INDC(I,J,K)
            IT=INDX(I,J,K  )
            IB=INDX(I,J,K-1)
...
C             * 正側の流速値
              ...
              IF (IS.GE.1) THEN
CBUG            IF (NF(I,J,K).NE.8) THEN
                IF (NF(I,J,K).NE.8 .AND. NF(I,J,K-1).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
...
C             * 負側の流速値
              ...
              IF (IS.GE.1) THEN
CBUG            IF (NF(I-1,J,K).NE.8) THEN
                IF (NF(I-1,J,K).NE.8 .AND. NF(I-1,J,K-1).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
