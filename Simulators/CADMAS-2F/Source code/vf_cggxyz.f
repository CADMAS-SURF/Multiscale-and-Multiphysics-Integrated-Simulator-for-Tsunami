            ...
            IF (MYGJS.LE.J .AND. J.LE.MYGJE) THEN
              ...
              DO 200 I=IPRARA(1,IB),IPRARA(4,IB)+1
                IF (LA.LE.I .AND. I.LE.LB) THEN
                  IW=I-IP
                  IF     (INDX(IW,JW,K).LE.-1) THEN
                    GGX(IW,JW,K)=1.0D0
                  ELSEIF (INDX(IW,JW,K).EQ. 0) THEN
                    GGX(IW,JW,K)=1.0D0
C@                  GGX(IW,JW,K)=MAX(GGV(IW,JW,K),GGV(IW-1,JW,K))
C@                  GGX(IW,JW,K)=MIN(GGV(IW,JW,K),GGV(IW-1,JW,K))
CN                  GGX(IW,JW,K)=XX(6,IW)* (XX(2,IW-1)*GGV(IW  ,JW,K)
CN   &                                     +XX(2,IW  )*GGV(IW-1,JW,K))
                  ENDIF
                ENDIF
 200          CONTINUE
            ENDIF
            ...
            IF (LA.LE.J .AND. J.LE.LB) THEN
              ...
              DO 300 I=IPRARA(1,IB),IPRARA(4,IB)
                IF (MYGIS.LE.I .AND. I.LE.MYGIE) THEN
                  IW=I-IP
                  IF     (INDY(IW,JW,K).LE.-1) THEN
                    GGY(IW,JW,K)=1.0D0
                  ELSEIF (INDY(IW,JW,K).EQ. 0) THEN
                    GGY(IW,JW,K)=1.0D0
C@                  GGY(IW,JW,K)=MAX(GGV(IW,JW,K),GGV(IW,JW-1,K))
C@                  GGY(IW,JW,K)=MIN(GGV(IW,JW,K),GGV(IW,JW-1,K))
CN                  GGY(IW,JW,K)=YY(6,JW)*( YY(2,JW-1)*GGV(IW,JW  ,K)
CN   &                                     +YY(2,JW  )*GGV(IW,JW-1,K))
                  ENDIF
                ENDIF
 300          CONTINUE
            ENDIF
          ...
          DO 410 J=IPRARA(2,IB),IPRARA(5,IB)
            IF (MYGJS.LE.J .AND. J.LE.MYGJE) THEN
              ...
              DO 400 I=IPRARA(1,IB),IPRARA(4,IB)
                IF (MYGIS.LE.I .AND. I.LE.MYGIE) THEN
                  IW=I-IP
                  IF     (INDZ(IW,JW,K).LE.-1) THEN
                    GGZ(IW,JW,K)=1.0D0
                  ELSEIF (INDZ(IW,JW,K).EQ. 0) THEN
                    GGZ(IW,JW,K)=1.0D0
C@                  GGZ(IW,JW,K)=MAX(GGV(IW,JW,K),GGV(IW,JW,K-1))
C@                  GGZ(IW,JW,K)=MIN(GGV(IW,JW,K),GGV(IW,JW,K-1))
CN                  GGZ(IW,JW,K)=ZZ(6,K)*( ZZ(2,K-1)*GGV(IW,JW,K  )
CN   &                                    +ZZ(2,K  )*GGV(IW,JW,K-1))
                  ENDIF
                ENDIF
 400          CONTINUE
            ENDIF
