      SUBROUTINE VF_K1CAL(XX,YY,ZZ,UU,VV,WW,FF,GGV,GGX,GGY,GGZ,GGV0,
...
CD    INDBE(MAXBE1,NUMB) : IN  : I*4 : 乱流エネルギ散逸の境界条件
      ...
      DIMENSION BCU(NUMB,3),BCV(NUMB,3),BCW(NUMB,3),BCVI(NUMB)
...
CD    -- 移流項によるフラックスの計算 --
     ...
     &                 FLKU,FLKV,FLKW,NF,INDX,INDY,INDZ,INDC,INDBK)
        CALL VF_SCONVD(-1,XX,YY,ZZ,UU,VV,WW,AE,GGX,GGY,GGZ,BCE,
     &                 FLEU,FLEV,FLEW,NF,INDX,INDY,INDZ,INDC,INDBE)
      ELSE
...
CD    -- 拡散項によるフラックスの計算 --
        ...
        DO 210 J=2,NUMJ-1
          DO 200 I=2,NUMI-1
            IF (NF(I,J,K).GT.-1) THEN
              ...
              WKNU(I,J,K)=ANU1+ANUT(I,J,K)*W
            ENDIF
 200      CONTINUE
 210    CONTINUE
 220  CONTINUE
      CALL VF_SDIFF(-1,XX,YY,ZZ,FF,AK,GGX,GGY,GGZ,BCK,WKNU,
     &              FLKU,FLKV,FLKW,INDX,INDY,INDZ,INDC)
      W=1.0D0/AKSGE
      DO 320 K=2,NUMK-1
        DO 310 J=2,NUMJ-1
          DO 300 I=2,NUMI-1
 ...
 310    CONTINUE
 320  CONTINUE
      CALL VF_SDIFF(-1,XX,YY,ZZ,FF,AE,GGX,GGY,GGZ,BCE,WKNU,
     &              FLEU,FLEV,FLEW,INDX,INDY,INDZ,INDC)
