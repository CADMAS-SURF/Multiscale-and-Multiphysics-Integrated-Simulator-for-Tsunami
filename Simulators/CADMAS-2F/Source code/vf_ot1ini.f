...
C     OPEN(MFILTR,ERR=9010,FILE='data.tran',
      OPEN(MFILTR,ERR=9010,FILE=TRIM(MGNAME(MGRANK+1))//'.tran',
C----------------------------------------------------for MG/2FC coupling
...
CD    -- 出力対象の出力 --
      DO 100 L=1,ITRNUM
        ...
        ELSEIF (ITRPRM(1,L).EQ.11) THEN
          TEXT1='FORCE'
        ELSE
          CALL VF_A2ERR('VF_OT1INI','P.G ERROR.')
        ENDIF
...
C==== ファイル関連エラー処理 =========================================
 ...
 9010 CONTINUE
C----------------------------------------------------for MG/2FC coupling
C     CALL VF_A2ERR('VF_OT1INI','CAN NOT OPEN (data.tran).')
      CALL VF_A2ERR('VF_OT1INI','CAN NOT OPEN ('
     &                  //TRIM(MGNAME(MGRANK+1))//'.tran).')
C----------------------------------------------------for MG/2FC coupling
      GOTO 9999
