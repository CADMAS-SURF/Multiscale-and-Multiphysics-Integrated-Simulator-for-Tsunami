...
C     * 時間間隔出力の場合
      ELSEIF (ILPTYP.EQ.2) THEN
        IF (TNOW.GE.RLPTRN(1)-ZERO .AND. TNOW.LE.RLPTRN(2)+ZERO) THEN
...
CD    -- ポーラス関連を出力する --
CSTR  IF (ILPON(5).NE.0 .AND. IPRNT.GT.1) THEN
      IF (ILPON(5).NE.0 .AND. 
     &    (IPRNT.GT.1 .OR. ICPL.GT.1 .OR. ISTM.EQ.1)) THEN
        ...
        CALL VF_OL3DR(GGZ,1)
      ENDIF
CSTR  IF (ILPON(6).NE.0 .AND. IPRNT.GT.1) THEN
      IF (ILPON(6).NE.0 .AND. 
     &    (IPRNT.GT.1 .OR. ICPL.GT.1 .OR. ISTM.EQ.1)) THEN
