...
CD    -- 粘性等の最大値を求める --
 ...
 120  CONTINUE
      IF (LEQT.NE.0) THEN
C2F     W=1.0D0/(RHO0*TCP0)
        DO 150 K=2,NUMK-1
          DO 140 J=MYJS,MYJE
            DO 130 I=MYIS,MYIE
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              RHO2=FF(I,J,K)*RHO0(1)+(1.0D0-FF(I,J,K))*RHO0(2)
!!            RHO2=FF(I,J,K)*RHO0(1)+(1.0D0-FF(I,J,K))*RHOG(I,J,K)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              W=1.0D0/(RHO2*TCP0)
...
C2F           IF (INDX(I  ,J,K).GE.1) THEN
C2F             IF (MAX(FC,BCF(INDX(I  ,J,K))).LE.0.0D0) FL1=0.0D0
C2F           ENDIF
C2F           IF (INDX(I+1,J,K).GE.1) THEN
C2F             IF (MAX(FC,BCF(INDX(I+1,J,K))).LE.0.0D0) FL2=0.0D0
C2F           ENDIF
C2F           IF (INDY(I,J  ,K).GE.1) THEN
C2F             IF (MAX(FC,BCF(INDY(I,J  ,K))).LE.0.0D0) FL3=0.0D0
C2F           ENDIF
C2F           IF (INDY(I,J+1,K).GE.1) THEN
C2F             IF (MAX(FC,BCF(INDY(I,J+1,K))).LE.0.0D0) FL4=0.0D0
C2F           ENDIF
C2F           IF (INDZ(I,J,K  ).GE.1) THEN
C2F             IF (MAX(FC,BCF(INDZ(I,J,K  ))).LE.0.0D0) FL5=0.0D0
C2F           ENDIF
C2F           IF (INDZ(I,J,K+1).GE.1) THEN
C2F             IF (MAX(FC,BCF(INDZ(I,J,K+1))).LE.0.0D0) FL6=0.0D0
C2F           ENDIF
