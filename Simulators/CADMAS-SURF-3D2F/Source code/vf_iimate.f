...
CD    -- MATE DENSITYを解釈する --
      ELSEIF (TEXT(IS(2):IE(2)).EQ.'DENSITY') THEN
        IF (NWD.LT.3) CALL VF_A2ERR('VF_IIMATE','SYNTAX ERROR.')
        CALL VF_ZSTOR(RHO0(1),TEXT(IS(3):IE(3)))
        IF (RHO0(1).LT.ZERO)
     &              CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
        IF (NWD.LT.4) THEN
          RHO0(2)=RHO0(1)
        ELSE
          CALL VF_ZSTOR(RHO0(2),TEXT(IS(4):IE(4)))
          IF (RHO0(2).LT.ZERO)
     &                CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
        ENDIF
...
CD    -- MATE K-VISCを解釈する --
      ELSEIF (TEXT(IS(2):IE(2)).EQ.'K-VISC') THEN
        IF (NWD.LT.3) CALL VF_A2ERR('VF_IIMATE','SYNTAX ERROR.')
        CALL VF_ZSTOR(ANU0(1),TEXT(IS(3):IE(3)))
        IF (ANU0(1).LT.0.0D0)
     &              CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
        IF (NWD.LT.4) THEN
          ANU0(2)=ANU0(1)
        ELSE
          CALL VF_ZSTOR(ANU0(2),TEXT(IS(4):IE(4)))
          IF (ANU0(2).LT.0.0D0)
     &                CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
        ENDIF
...
C       * MATE I.C. V
        IF     (TEXT(IS(3):IE(3)).EQ.'V') THEN
          ...
          CALL VF_ZSTOR(UINI(1),TEXT(IS(4):IE(4)))
          CALL VF_ZSTOR(VINI(1),TEXT(IS(5):IE(5)))
          CALL VF_ZSTOR(WINI(1),TEXT(IS(6):IE(6)))
C       * MATE I.C. VG
        ELSEIF (TEXT(IS(3):IE(3)).EQ.'VG') THEN
          IF (NWD.LT.6) CALL VF_A2ERR('VF_IIMATE','SYNTAX ERROR.')
          CALL VF_ZSTOR(UINI(2),TEXT(IS(4):IE(4)))
          CALL VF_ZSTOR(VINI(2),TEXT(IS(5):IE(5)))
          CALL VF_ZSTOR(WINI(2),TEXT(IS(6):IE(6)))
C       * MATE I.C. KE
        ELSEIF (TEXT(IS(3):IE(3)).EQ.'KE') THEN
          IF (NWD.LT.5) CALL VF_A2ERR('VF_IIMATE','SYNTAX ERROR.')
          CALL VF_ZSTOR(AKINIK(1),TEXT(IS(4):IE(4)))
          CALL VF_ZSTOR(AKINIE(1),TEXT(IS(5):IE(5)))
          IF (AKINIK(1).LT.ZERO)
     &                 CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
          IF (AKINIE(1).LT.ZERO)
     &                 CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
C       * MATE I.C. KG
        ELSEIF (TEXT(IS(3):IE(3)).EQ.'KG') THEN
          IF (NWD.LT.5) CALL VF_A2ERR('VF_IIMATE','SYNTAX ERROR.')
          CALL VF_ZSTOR(AKINIK(2),TEXT(IS(4):IE(4)))
          CALL VF_ZSTOR(AKINIE(2),TEXT(IS(5):IE(5)))
          IF (AKINIK(2).LT.ZERO)
     &                 CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
          IF (AKINIE(2).LT.ZERO)
     &                 CALL VF_A2ERR('VF_IIMATE','INVALID VALUE.')
