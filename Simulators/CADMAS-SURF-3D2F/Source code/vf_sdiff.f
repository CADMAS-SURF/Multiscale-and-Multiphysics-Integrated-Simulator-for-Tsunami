      SUBROUTINE VF_SDIFF(ISW,XX,YY,ZZ,FF,CC,GGX,GGY,GGZ,BCC,DD,
     &                    FLCU,FLCV,FLCW,INDX,INDY,INDZ,INDC)
...
CD    INDC(@FOR-3D@)   : IN  : I*4 : セルの計算状態を示すインデックス
      ...
      DIMENSION INDZ(NUMI,NUMJ,NUMK),INDC(NUMI,NUMJ,NUMK)
...
CD    -- 初期設定 --
      ...
      ELSEIF (ISW.EQ. 0) THEN
        AL(1)=1.0D0/(RHO0(1)*TCP0)
        AL(2)=1.0D0/(RHO0(2)*TCP0)
...
C2F         IF (NF(I-1,J,K)*NF(I,J,K).EQ.0) THEN
            IF (INDC(I-1,J,K)*INDC(I,J,K).EQ.0) THEN
              IND=INDX(I,J,K)
              AL2=FF(I,J,K)*AL(1)+(1.0D0-FF(I,J,K))*AL(2)
...
C2F         IF (NF(I,J-1,K)*NF(I,J,K).EQ.0) THEN
            IF (INDC(I,J-1,K)*INDC(I,J,K).EQ.0) THEN
              IND=INDY(I,J,K)
              AL2=FF(I,J,K)*AL(1)+(1.0D0-FF(I,J,K))*AL(2)
...
C2F         IF (NF(I,J,K-1)*NF(I,J,K).EQ.0) THEN
            IF (INDC(I,J,K-1)*INDC(I,J,K).EQ.0) THEN
              IND=INDZ(I,J,K)
              AL2=FF(I,J,K)*AL(1)+(1.0D0-FF(I,J,K))*AL(2)
