...
C2F         IC=NF(I-1,J,K-1)*NF(I-1,J,K)*NF(I,J,K-1)*NF(I,J,K)
            IC=INDC(I-1,J,K-1)*INDC(I-1,J,K)*INDC(I,J,K-1)*INDC(I,J,K)
            IT=INDZ(I  ,J,K)
            IB=INDZ(I-1,J,K)
...
C             * 負側の流速値
              IS=INDX(I,J,K-1)
              IF (IS.GE.1) THEN
CBUG            IF (NF(I,J,K-1).NE.8) THEN
                IF (NF(I,J,K-1).NE.8 .AND. NF(I-1,J,K-1).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
...
C             ? 線形補間
              GG=ZZ(6,K)*(ZZ(2,K-1)*GGX(I,J,K)+ZZ(2,K)*GGX(I,J,K-1))
CMVO          FG=ZZ(6,K)*(ZZ(2,K)*BR+ZZ(2,K-1)*BL)
              FG=ZZ(6,K)*(ZZ(2,K)*BR2+ZZ(2,K-1)*BL2)
...
CD    -- (Gy*v)*wとGy*Nu*(dw/dy+dv/dz)の計算 --
      DO 320 K=3,NUMK-1
        DO 310 J=MYJS,MYJE+1
          DO 300 I=MYIS,MYIE
C2F         IC=NF(I,J-1,K-1)*NF(I,J-1,K)*NF(I,J,K-1)*NF(I,J,K)
            IC=INDC(I,J-1,K-1)*INDC(I,J-1,K)*INDC(I,J,K-1)*INDC(I,J,K)
            IT=INDZ(I,J  ,K)
            IB=INDZ(I,J-1,K)
...
C             * 負側の流速値
              ...
              IF (IS.GE.1) THEN
CBUG            IF (NF(I,J,K-1).NE.8) THEN
                IF (NF(I,J,K-1).NE.8 .AND. NF(I,J-1,K-1).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
