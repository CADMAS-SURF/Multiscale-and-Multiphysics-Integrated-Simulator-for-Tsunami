      SUBROUTINE VF_BWKELG(XX,YY,ZZ,UU,VV,WW,FF,BCVI,AK,AE,DBUF,
...
C2F         IF (NF(I,J,K).EQ.0) THEN
            ...
            IF (N.GT.-1) THEN
              ...
              IZM=0
              IZP=0
              IF (LXM.GE.1) IXM=INDB(LGBF,LXM)
              IF (LXP.GE.1) IXP=INDB(LGBF,LXP)
              IF (LYM.GE.1) IYM=INDB(LGBF,LYM)
              IF (LYP.GE.1) IYP=INDB(LGBF,LYP)
              IF (LZM.GE.1) IZM=INDB(LGBF,LZM)
              IF (LZP.GE.1) IZP=INDB(LGBF,LZP)
     ...
     &            IZM.EQ.8 .OR. IZP.EQ.8     ) THEN
                ...
                SX=0.0D0
                UT=0.0D0
                ANU1=FF(I,J,K)*ANU0(1)+(1.D0-FF(I,J,K))*ANU0(2)
                IF (IXM.EQ.6 .OR. IXP.EQ.6) THEN
                  IF (VX.GE.ZERO .AND. ANU1.GE.ZERO)
     &                                 CALL VF_CLOGLW(DX,VX,ANU1,UT)
                  SX=1.0D0
                ...
                IF (IYM.EQ.6 .OR. IYP.EQ.6) THEN
                  IF (VY.GE.ZERO .AND. ANU1.GE.ZERO)
     &                                 CALL VF_CLOGLW(DY,VY,ANU1,VT)
                  SY=1.0D0
                ...
                IF (IZM.EQ.6 .OR. IZP.EQ.6) THEN
                  IF (VZ.GE.ZERO .AND. ANU1.GE.ZERO)
     &                                 CALL VF_CLOGLW(DZ,VZ,ANU1,WT)
                  SZ=1.0D0
                ...
                IF (IXM.EQ.8) THEN
                  IF (VX.GE.ZERO)
     &                      CALL VF_CLOGKS(DX,VX,BCVI(LXM),ANU1,UTM)
                  SXM=1.0D0
                ENDIF
                SXP=0.0D0
                UTP=0.0D0
                IF (IXP.EQ.8) THEN
                  IF (VX.GE.ZERO)
     &                      CALL VF_CLOGKS(DX,VX,BCVI(LXP),ANU1,UTP)
                  SXP=1.0D0
                ENDIF
                SYM=0.0D0
                VTM=0.0D0
                IF (IYM.EQ.8) THEN
                  IF (VY.GE.ZERO)
     &                      CALL VF_CLOGKS(DY,VY,BCVI(LYM),ANU1,VTM)
                  SYM=1.0D0
                ENDIF
                SYP=0.0D0
                VTP=0.0D0
                IF (IYP.EQ.8) THEN
                  IF (VY.GE.ZERO)
     &                      CALL VF_CLOGKS(DY,VY,BCVI(LYP),ANU1,VTP)
                  SYP=1.0D0
                ENDIF
                SZM=0.0D0
                WTM=0.0D0
                IF (IZM.EQ.8) THEN
                  IF (VZ.GE.ZERO)
     &                      CALL VF_CLOGKS(DZ,VZ,BCVI(LZM),ANU1,WTM)
                  SZM=1.0D0
                ENDIF
                SZP=0.0D0
                WTP=0.0D0
                IF (IZP.EQ.8) THEN
                  IF (VZ.GE.ZERO)
     &                      CALL VF_CLOGKS(DZ,VZ,BCVI(LZP),ANU1,WTP)
                  SZP=1.0D0
                ENDIF
