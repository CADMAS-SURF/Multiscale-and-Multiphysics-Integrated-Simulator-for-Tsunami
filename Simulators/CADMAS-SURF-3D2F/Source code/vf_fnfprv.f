...
CD      -- 方向が決定しなければエラー --
C2F     IF (NF(I,J,K).EQ.7) CALL VF_A2ERR('VF_FNFPRV','P.G ERROR.')
        IF (NF(I,J,K).EQ.7) THEN
          WRITE(*,'(A,3(I4,:,'',''))')
     &                'VF_FNFPRV> P.G ERROR. I,J,K=',I,J,K
          DO JD=J-1,J+1
            WRITE(*,'(A,I4,A,3(I12,:,'',''))') 'J=',JD,' I=',I-1,I,I+1
            DO KD=K+1,K-1,-1
              WRITE(*,'(A,I4,A,1P,3(E12.4,:,'',''))')
     &                    '  K=',KD,':',(FF(ID,JD,KD),ID=I-1,I+1)
            ENDDO
          ENDDO
          CALL VF_A2ERR('VF_FNFPRV','P.G ERROR.')
        ENDIF
