        ...
        DO 110 KC=KS,KE
          ...
          DO 100 JC=JS,JE
            ...
            IF (L.GT.0) THEN
              ...
              BCW(L,:) = BC(4)
              UU(I,JC,KC) = BCU(L,1)
              VV(I+IP,JC,KC) = BCV(L,1) + (YPF(JC) - Y0)*BC(8)
              IF(JC.EQ.JE) VV(I+IP,JC+1,KC) = BCV(L,1) + 0.5D0*BC(8)
              WW(I+IP,JC,KC) = BCW(L,1) + (ZPF(KC) - Z0)*BC(9)
              IF(KC.EQ.KE) WW(I+IP,JC,KC+1) = BCW(L,1) + 0.5D0*BC(9)
            ENDIF
 100      CONTINUE
 110    CONTINUE
C
        ...
        DO 210 KC=KS,KE
          ...
          DO 200 IC=IS,IE
            ...
            IF (L.GT.0) THEN
              ...
              BCW(L,:) = BC(4)
              VV(IC,J,KC) = BCV(L,1)
              UU(IC,J+IP,KC) = BCU(L,1) + (XPF(IC) - X0)*BC(8)
              IF(IC.EQ.IE) UU(IC+1,J+IP,KC) = BCU(L,1) + 0.5D0*BC(8)
              WW(IC,J+IP,KC) = BCW(L,1) + (ZPF(KC) - Z0)*BC(9) 
              IF(KC.EQ.KE) WW(IC,J+IP,KC+1) = BCW(L,1) + 0.5D0*BC(9)
            ENDIF
 200      CONTINUE
 210    CONTINUE
C        
