...
CD    -- MODEL PORO-BCを解釈する --
      ELSEIF (TEXT(IS(2):IE(2)).EQ.'PORO-BC') THEN
        IF (NWD.LT.4) CALL VF_A2ERR('VF_IIMDL','SYNTAX ERROR.')
        IF     (TEXT(IS(3):IE(3)).EQ.'VP') THEN
          IF     (TEXT(IS(4):IE(4)).EQ.'SLIP') THEN
            IPRTB(1)=1
          ELSEIF (TEXT(IS(4):IE(4)).EQ.'NON-S') THEN
            IPRTB(1)=2
          ELSE
            CALL VF_A2ERR('VF_IIMDL','UNKNOWN WORD.')
          ENDIF
...
CDBG>
        WRITE(*,'(A,3I3)')
     &    ' VF_IIMDL> PORO-BC: IPRTB=',(IPRTB(I),I=1,3)
CDBG<
