...
! Common Definition
  ...
  integer,parameter:: &                            ! モデル番号の定義
       ...
       &             ,l_mlt_agent =13, l_str       =14, l_stm     =15
...
! Definition for each model
  ...
  integer,parameter:: my_model=l_cadmas_2fc
!
! Global variables
...
!                                  ! 1対1通信のみであれば、mpi_comm_worldやcomm_groupを用いてもよい。
  ...
  integer:: comm_2fc_stm           ! CADMAS-SURF/2FCとSTMの通信に用いるコミュニケータ
...
!                                    COMM_SPLITに参加するモデルを絞ったコミュニケータを作成しておく)
  ...
  integer:: comm_work_2fc_stm      ! CADMAS-SURF/2FCとSTMを含むコミュニケータ
...
!
    imodel=my_model
    if( imodel==l_cadmas_2fc.or.imodel==l_stm ) then
       imodel=0
    else
       imodel=1
    endif
    call mpi_comm_split(mpi_comm_world,imodel,nrank_all,comm_work_2fc_stm &   ! comm_work_2fc_stmの作成
         &            ,ierr)
         if(ierr/=0) goto 911
!
    imodel=my_model
