...
C2F         IC=NF(I-1,J-1,K)*NF(I-1,J,K)*NF(I,J-1,K)*NF(I,J,K)
            IC=INDC(I-1,J-1,K)*INDC(I-1,J,K)*INDC(I,J-1,K)*INDC(I,J,K)
            IT=INDY(I  ,J,K)
            IB=INDY(I-1,J,K)
...
CBUG            IF (NF(I,J,K).NE.8) THEN
                IF (NF(I,J,K).NE.8 .AND. NF(I-1,J,K).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
...
C             * 負側の流速値
              IS=INDX(I,J-1,K)
              IF (IS.GE.1) THEN
CBUG            IF (NF(I,J-1,K).NE.8) THEN
                IF (NF(I,J-1,K).NE.8 .AND. NF(I-1,J-1,K).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
...
C             ? 線形補間
              GG=YY(6,J)*(YY(2,J-1)*GGX(I,J,K)+YY(2,J)*GGX(I,J-1,K))
CMVO          FG=YY(6,J)*(YY(2,J)*BR+YY(2,J-1)*BL)
              FG=YY(6,J)*(YY(2,J)*BR2+YY(2,J-1)*BL2)
              IF     (NF(I  ,J,K).LE.-1) THEN
                ANR=ANU(I-1,J,K)
              ELSEIF (NF(I-1,J,K).LE.-1) THEN
                ANR=ANU(I  ,J,K)
              ELSE
                ANR=XX(6,I)*( XX(2,I-1)*ANU(I  ,J,K)
     &                       +XX(2,I  )*ANU(I-1,J,K))
              ENDIF
              IF     (NF(I  ,J-1,K).LE.-1) THEN
                ANL=ANU(I-1,J-1,K)
...
CD    -- (Gz*w)*vとGz*Nu*(dv/dz+dw/dy)の計算 --
      DO 320 K=2,NUMK
        DO 310 J=LA,LB
          DO 300 I=MYIS,MYIE
C2F         IC=NF(I,J-1,K-1)*NF(I,J,K-1)*NF(I,J-1,K)*NF(I,J,K)
            IC=INDC(I,J-1,K-1)*INDC(I,J,K-1)*INDC(I,J-1,K)*INDC(I,J,K)
            IT=INDY(I,J,K  )
            IB=INDY(I,J,K-1)
...
C             * 負側の流速値
              ...
              IF (IS.GE.1) THEN
CBUG            IF (NF(I,J-1,K).NE.8) THEN
                IF (NF(I,J-1,K).NE.8 .AND. NF(I,J-1,K-1).NE.8) THEN
                  LGBF=3
                ELSE
                  LGBF=5
                ENDIF
